...
- **Parallel safe**: Works with parallel scanning (`scan-all --workers`)
...
- `java` - Full support with annotations, Spring, Lombok (v0.7.0+)
...
```yaml
# No ai_command specified — scan defaults to structural mode
languages:
  - python
include:
  - src/
...
```bash
codeindex scan .
# Structural documentation is generated by default (no AI needed)
```