...
## 关键特性
1. **调用关系提取** (v0.12.0): 函数/方法调用图谱，依赖分析
2. **智能符号评分** (v0.2.0): 5维度评分系统，重要符号优先
3. **自适应符号提取** (v0.2.0): 根据文件大小动态调整符号数量
4. **框架路由提取** (v0.5.0): ThinkPHP, Spring 自动路由提取
5. **分层文档生成**: 三级智能索引系统
6. **AI增强**: 支持Claude、OpenAI等AI CLI
7. **并行处理**: 支持多目录并发扫描
8. **自动化发布**: Makefile + GitHub Actions + PyPI Trusted Publisher
