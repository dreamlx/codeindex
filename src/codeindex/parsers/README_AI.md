<!-- Generated by codeindex at 2026-02-08T11:28:19+08:00 -->
...
- All parsers check `tree.root_node.has_error` after parsing
- Returns `ParseResult` with error message if syntax errors detected
- Includes `file_lines` count for context
- Prevents extraction attempts on malformed code
...
- **Syntax Validation**: Detects Python syntax errors before symbol extraction
...
- **Syntax Validation**: Detects PHP syntax errors before symbol extraction
...
- **Syntax Validation**: Detects Java syntax errors before symbol extraction
- **Advanced Features**:
...
All language parsers implement early syntax error detection:

1. **Parse with tree-sitter**: Generate AST from source bytes
2. **Check for errors**: Inspect `tree.root_node.has_error`
3. **Return error result**: If syntax errors detected:
   ```python
   return ParseResult(
       path=path,
   ...
   ```
4. **Continue extraction**: Only if AST is valid

This prevents extraction attempts on malformed code and provides clear error feedback.
...
6. Implement syntax error detection (check `tree.root_node.has_error`)
...
- **Early Validation**: Syntax errors detected before extraction attempts
...
- **Early Error Detection**: Syntax validation prevents wasted extraction work
...
- **v0.13.1**: Added syntax error detection for all language parsers
