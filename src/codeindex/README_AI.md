<!-- Generated by codeindex at 2026-01-27T18:52:32+08:00 -->
...
Changed files:
- `ai_helper.py` (new file)

Key changes:
- Added new `ai_helper.py` module providing reusable AI enhancement helper functions
- Implemented `aggregate_parse_results()` function to combine multiple ParseResult objects into one aggregated result with summed file_lines and collected symbols
- Implemented `execute_multi_turn_enhancement()` function providing unified interface for multi-turn dialogue enhancement
- `execute_multi_turn_enhancement()` supports three strategies: "auto" (auto-detect super large files), "multi_turn" (force multi-turn), "standard" (skip multi-turn)
- Returns tuple of (success, WriteResult, message) for consistent handling across CLI commands
- Eliminates code duplication between `scan` and `scan-all` commands
- Integrates with `is_super_large_file()` for detection and `multi_turn_ai_enhancement()` for execution
- Handles graceful fallback when multi-turn dialogue fails or is not applicable
