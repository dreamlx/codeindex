<!-- Generated by codeindex at 2026-02-06T09:12:11+08:00 -->
...
Supports Python, PHP, and Java languages with full symbol extraction including
classes, functions, methods, fields, imports, docstrings, and throws declarations._
...
**Commit `6a637ee`**: feat(epic10): Story 10.2.1 - Python import alias extraction
...
- `_parse_import` now returns `list[Import]` instead of `Import | None`, creating separate Import objects for each imported item with alias support
- Aliased imports (`import numpy as np`, `from typing import Dict as D`) are properly extracted with the `alias` field on `Import`
- `parse_file` updated to use `imports.extend()` to consume the new list return type
