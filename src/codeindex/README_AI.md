<!-- Generated by codeindex at 2026-01-26T14:57:49+08:00 -->
...
## Architecture
The tool follows a pipeline architecture where directories are scanned, parsed, formatted into prompts, and sent to external AI CLI tools for documentation generation. It supports incremental updates, parallel processing, hierarchical organization with two-phase AI enhancement, intelligent symbol scoring for prioritization, and adaptive symbol extraction.

Data Flow:
```
Directory (Scanner) → Files (Scanner) → Parsed Data (Parser) → 
Symbol Scoring (SymbolImportanceScorer) → Adaptive Symbol Selection (AdaptiveSymbolsConfig) → 
...
| `AdaptiveSymbolsConfig` | Configuration for adaptive symbol extraction based on file size categories |
...
| Adaptive Symbol Extraction | Dynamically adjusts symbol count based on file size categories (tiny/small/medium/large/xlarge/huge/mega) with configurable thresholds and limits |
...
| Adaptive Configuration | File size-based symbol extraction with 7 categories (tiny: <100 lines/10 symbols, small: <200/15, medium: <500/30, large: <1000/50, xlarge: <2000/80, huge: <5000/120, mega: ≥5000/150); configurable min/max bounds (5-200 symbols); backward compatible (disabled by default) |
