<!-- Generated by codeindex at 2026-01-27T19:52:28+08:00 -->
...
| Module | Role |
|--------|------|
| `cli.py` | Main CLI entry point with Click command definitions; imports shared resources from `cli_common` and registers commands from submodules |
| `cli_common.py` | Common utilities shared across CLI modules including the Rich console instance for formatted output |
| `cli_tech_debt.py` | Technical debt analysis command module providing `tech-debt` command with file discovery, analysis, and multi-format reporting |
...
| `tech_debt` | `codeindex tech-debt <path> [--format] [--output] [--recursive]` | Analyze technical debt in directory with configurable output format (implemented in `cli_tech_debt.py`) |

### Helper Functions

| Function | Module | Purpose |
|----------|--------|---------|
| `_find_source_files` | `cli_tech_debt.py` | Find source files in directory based on language configuration; supports multiple languages with extension mapping; returns list of Path objects |
| `_analyze_files` | `cli_tech_debt.py` | Parse and analyze source files for technical debt; integrates parser, scorer, and detector; adds results to reporter |
| `_format_and_output` | `cli_tech_debt.py` | Format technical debt report using selected formatter (console/markdown/json); write to file or stdout |
| `is_super_large_file` | `ai_enhancement.py` | Detect if file exceeds super large thresholds using unified `FileSizeClassifier` (Epic 4 refactoring); returns detection result with recommended strategy based on `FileSizeCategory` |
| `select_enhancement_strategy` | `ai_enhancement.py` | Select appropriate AI enhancement strategy based on file size: standard (<2000 lines, <40 symbols), hierarchical (2000-5000 lines, 40-100 symbols), or multi_turn (>5000 lines OR >100 symbols) |
| `multi_turn_ai_enhancement` | `ai_enhancement.py` | Execute 3-round multi-turn dialogue for super large files; returns `MultiTurnResult` with final README content |
| `aggregate_parse_results` | `ai_helper.py` | Combine multiple ParseResult objects into one aggregated result; sums file_lines and collects all symbols for multi-file directory analysis |
| `execute_multi_turn_enhancement` | `ai_helper.py` | Unified interface for multi-turn dialogue enhancement with auto-detection, execution, and fallback logic; eliminates code duplication between scan and scan-all commands |
...
| Modular CLI Architecture | CLI split into `cli.py` (main entry point), `cli_common.py` (shared utilities), and `cli_tech_debt.py` (tech-debt command) for better maintainability and future command module expansion (Epic 4) |
| Multi-Language Tech Debt Analysis | `_find_source_files()` supports 9 languages (Python, PHP, JavaScript, TypeScript, Java, Go, Rust, C++, C) via extension mapping |
...
| `codeindex.cli_common` | Shared CLI utilities including `console` instance for formatted output |
| `codeindex.cli_tech_debt` | Technical debt CLI command module providing `tech_debt` command |
...
| Technical Debt CLI | `tech-debt` command provides standalone technical debt analysis (via `cli_tech_debt.py` module) |
...
Changed files:
- `cli.py`
- `cli_tech_debt.py` (new)

Key changes:
- Extracted technical debt CLI command and helper functions to new `cli_tech_debt.py` module
- Moved `_find_source_files()`, `_analyze_files()`, `_format_and_output()`, and `tech_debt` command from `cli.py` to `cli_tech_debt.py`
- `cli.py` now imports and registers `tech_debt` command using `main.add_command(tech_debt)`
- Removed direct imports of `ScoringContext`, `SymbolImportanceScorer`, `TechDebtDetector`, `TechDebtReport`, `TechDebtReporter`, and formatters from `cli.py`
- `cli_tech_debt.py` imports shared `console` from `cli_common` for consistent output formatting
- Reduces `cli.py` by ~230 lines, improving maintainability
- Continues Epic 4 CLI modularization effort, enabling future extraction of additional command modules

**Commit `36348fb`**: feat(epic4): create cli_common module and BDD tests for CLI split
...
Key changes:
- Created new `cli_common.py` module to hold shared CLI utilities
- Moved `console` instance (Rich Console) from `cli.py` to `cli_common.py`
- `cli.py` now imports `console` from `cli_common` instead of creating its own instance
- Removed direct `rich.console.Console` import from `cli.py`
- Prepares CLI architecture for future modular command splitting (Epic 4)
- Enables sharing of console and other common utilities across multiple CLI command modules
