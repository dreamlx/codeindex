...
- `CLAUDE_MD_SECTION` - Template content injected between markers, containing codeindex usage instructions for AI agents. Includes:
  - **First-time setup** guidance: review `.codeindex.yaml`, run `codeindex scan-all`, optional hooks install
  - **Daily usage** instructions: read README_AI.md first, run `codeindex scan` for missing/outdated docs, check coverage with `codeindex status`
...
- Shows a 3-step flow: (1) Review `.codeindex.yaml` config, (2) Generate documentation indexes with `scan-all`, (3) Check coverage with `status`
