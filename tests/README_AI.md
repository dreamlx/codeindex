<!-- Generated by codeindex at 2026-01-27T15:06:44+08:00 -->
...
- `test_tech_debt_bdd.py` provides BDD-style tests using pytest-bdd with Gherkin feature files for scenario-driven technical debt and symbol overload validation
...
- BDD step definitions: `@given`, `@when`, `@then` steps for scenario-driven technical debt and symbol overload testing with parameterized fixtures for file data, language types, symbol counts, class structures, and symbol overload analysis results
...
| BDD scenario tests | Validates technical debt detection and symbol overload detection through Gherkin scenarios with given-when-then steps, parameterized test data, behavior-driven validation, and flexible fixture resolution using pytest request object |
